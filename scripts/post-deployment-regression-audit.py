...
ROUTES = PUBLIC_ROUTES + SYSTEM_ROUTES
MOBILE_WIDTHS = [
...
with sync_playwright() as p:
    ...
    for route in ROUTES:
        ...
        page.on(
            "console",
            lambda msg, r=route: console_messages.append({"route": r, "type": msg.type, "text": msg.text})
            if msg.type in {"error", "warning"} else None,
        )
        page.on(
            "pageerror",
            lambda err, r=route: console_messages.append({"route": r, "type": "pageerror", "text": str(err)}),
        )
        ...
        try:
            ...
            raw_response = request.get(f"{BASE}{route}", fail_on_status_code=False)
            ...
            metrics = page.evaluate("""() => {
              ...
              return {
                ...
                unnamed:[...document.querySelectorAll('a[href],button,input,select,textarea')]
                  .filter(visible)
                  .filter(el=>!(el.getAttribute('aria-label')||el.getAttribute('title')||(el.innerText||'').trim()||el.getAttribute('alt')))
                  .map(el=>({tag:el.tagName,cls:String(el.className||''),type:el.getAttribute('type')})),
            ...
            route_records.append({
                "route": route,
                "status": status,
                "headers": {key: headers.get(key) for key in required_headers},
                "metrics": metrics,
            })
            ...
            if metrics["lang"] != "en" or metrics["h1"] != 1 or metrics["main"] != 1 or not metrics["skipLink"]:
                add(
                    ...
                    {key: metrics[key] for key in ["lang", "h1", "main", "skipLink"]},
                    route=route,
                )
            if metrics["images"]:
            ...
            if metrics["canonical"] != canonical_expected or metrics["ogUrl"] != canonical_expected:
                add(
                    ...
                    {"expected": canonical_expected, "canonical": metrics["canonical"], "ogUrl": metrics["ogUrl"]},
                    route=route,
                )
            ...
            if re.search(r"<img\b[^>]*\s/\s+style=", raw, re.I):
                add(
                    "medium",
                    "malformed-img-start-tag",
                    "A self-closing slash appears before a later style attribute.",
                    route=route,
                )
        except Exception as exc:
        ...
        for route in ROUTES:
            ...
            try:
                ...
                metrics = page.evaluate("""() => {
                  ...
                  const visible=el=>{const s=getComputedStyle(el),x=el.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&Number(s.opacity||1)>.01&&x.width>0&&x.height>0};
                ...
                page.screenshot(path=str(SHOTS / f"menu-{screenshot_name}--{label}.png"), full_page=False)
                page.keyboard.press("Escape")
                page.wait_for_timeout(100)
                metrics["closedByEscape"] = toggle.get_attribute("aria-expanded") == "false"
                menu_records.append({"route": route, "viewport": label, **metrics})
                minimum_height = max(250, height - metrics["rect"]["top"] - 8)
                if not metrics["open"] or metrics["expanded"] != "true" or not metrics["bodyLocked"] or not metrics["closedByEscape"]:
                    add("high", "mobile-menu-state-contract", metrics, route=route, viewport=label)
                if len(metrics["visibleTop"]) < 8:
                    add("critical", "mobile-menu-items-hidden", metrics, route=route, viewport=label)
                if metrics["rect"]["height"] < minimum_height or metrics["rect"]["bottom"] < height - 4:
                    add(
                        "critical",
                        "mobile-menu-collapsed-height",
                        {"minimum": minimum_height, **metrics},
                        route=route,
                        viewport=label,
                    )
                if not metrics["firstFocused"]:
                    add("medium", "mobile-menu-initial-focus", metrics, route=route, viewport=label)
            except Exception as exc:
        ...
        for route in FORM_ROUTES:
            ...
            try:
                ...
                record = {
                    ...
                    "staleDescriptionIdsAfterCorrection": stale,
                }
    ...
    request_record(
        "contact-wrong-content-type",
        request.post(f"{BASE}/api/contact", data="x", headers={"Content-Type": "text/plain"}, fail_on_status_code=False),
        415,
    )
    request_record(
        "contact-empty-json",
        request.post(f"{BASE}/api/contact", data="{}", headers={"Content-Type": "application/json"}, fail_on_status_code=False),
        400,
    )
    request_record(
        "project-empty-json",
        request.post(f"{BASE}/api/project", data="{}", headers={"Content-Type": "application/json"}, fail_on_status_code=False),
        400,
    )
    request_record(
        "document-empty-json",
        request.post(f"{BASE}/api/document-request", data="{}", headers={"Content-Type": "application/json"}, fail_on_status_code=False),
        400,
    )
    request_record(
        "contact-cross-site",
    ...
    if manifest.status != 200 or not purposes or any(purpose != "any" for purpose in purposes):
    ...
    if sitemap.status != 200 or len(sitemap_urls) != len(PUBLIC_ROUTES) or len(set(sitemap_urls)) != len(PUBLIC_ROUTES):
        add(
            "high",
            "sitemap-contract",
            {"status": sitemap.status, "count": len(sitemap_urls), "unique": len(set(sitemap_urls))},
        )
