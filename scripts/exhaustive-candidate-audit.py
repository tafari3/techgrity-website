...
JS=r'''async () => {
 ...
 const bgFor=el=>{const layers=[];for(let n=el;n;n=n.parentElement){const c=rgba(getComputedStyle(n).backgroundColor);if(c&&c[3]>0){layers.push(c);if(c[3]>=.999)break}}let base=[255,255,255];for(let i=layers.length-1;i>=0;i--){const c=layers[i],a=c[3];base=[c[0]*a+base[0]*(1-a),c[1]*a+base[1]*(1-a),c[2]*a+base[2]*(1-a)]}return base;};
 const contrast=[];
