...
function normaliseHtml(input){
 ...
 html=html.replace(/<img src="\/assets\/techgrity-logo-light\.svg"([^>]*)>/g,`<img src="/assets/techgrity-logo-light.svg"$1 style="${FOOTER_LOGO_STYLE}">`);
...
rm(dist);copy(path.join(root,'public'),dist);writeBrandBinaryAssets(path.join(dist,'assets'));finalizeBrandAssets(path.join(dist,'assets'));
