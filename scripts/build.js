...
const BRAND_BUILD=path.join(root,'build','techgrity-corporate-web-assets');
const PRIMARY_LOGO='/assets/techgrity-horizontal-primary-1600.png';
const REVERSED_LOGO='/assets/techgrity-horizontal-reversed-1600.png';
const SYMBOL_LOGO='/assets/techgrity-symbol-primary-512.png';
const FORMAL_BRAND_FILES=['techgrity-horizontal-primary-1600.png','techgrity-horizontal-reversed-1600.png','techgrity-symbol-primary-512.png'];
const FOOTER_LOGO_STYLE='width:220px!important;max-width:100%!important;height:auto!important;padding:0!important;background:transparent!important;border:0!important;border-radius:0!important;box-shadow:none!important';
const ICON_LINKS=`<link rel="icon" type="image/png" sizes="512x512" href="${SYMBOL_LOGO}"><link rel="apple-touch-icon" sizes="512x512" href="${SYMBOL_LOGO}">`;
...
function normaliseHtml(input){
 ...
 html=html.replaceAll('/assets/techgrity-primary-horizontal-approved.png',PRIMARY_LOGO);
 html=html.replaceAll('/assets/techgrity-logo.svg',PRIMARY_LOGO);
 html=html.replaceAll('/assets/techgrity-logo-light.svg',REVERSED_LOGO);
 html=html.replace(/(<div class="footer-brand"[^>]*>\s*<img src=")[^"]+("[^>]*>)/g,`$1${REVERSED_LOGO}$2`);
 html=html.replace(/<div class="footer-brand">/g,'<div class="footer-brand" style="background:transparent!important;border:0!important;border-radius:0!important;box-shadow:none!important;padding:0!important">');
 html=html.replace(new RegExp(`<img src="${REVERSED_LOGO.replaceAll('/','\\/')}"([^>]*)>`,'g'),`<img src="${REVERSED_LOGO}"$1 style="${FOOTER_LOGO_STYLE}">`);
...
for(const stale of ['techgrity-logo.svg','techgrity-logo-light.svg','techgrity-mark.svg'])rm(path.join(dist,'assets',stale));
for(const filename of FORMAL_BRAND_FILES){const source=path.join(BRAND_BUILD,filename);if(!fs.existsSync(source))throw new Error(`Missing governed corporate derivative: ${filename}`);fs.copyFileSync(source,path.join(dist,'assets',filename))}
fs.copyFileSync(path.join(BRAND_BUILD,'provenance.json'),path.join(dist,'assets','techgrity-corporate-provenance.json'));
...
home=home.replace(/<script type="application\/ld\+json">[\s\S]*?<\/script>/,`<script type="application/ld+json">${JSON.stringify({'@context':'https://schema.org','@type':'Organization',name:'Techgrity Systems',legalName:'TECHGRITY SYSTEMS (PRIVATE) LIMITED',url:'https://techgrity.co.zw/',logo:`https://techgrity.co.zw${PRIMARY_LOGO}`,email:'business@techgrity.co.zw',telephone:PHONE_DISPLAY,address:{'@type':'PostalAddress',streetAddress:'2367 Lavenham Drive',addressLocality:'Westgate, Harare',addressCountry:'ZW'},areaServed:['Zimbabwe','Africa'],description:'Techgrity Systems delivers digital systems, critical infrastructure and integrated technology solutions.'})}</script>`);
writeRoute('/',home);
const pages=[];
...
console.log(`Built ${publicRoutes.length} public routes and ${pages.filter(p=>!p.indexable).length} system routes with founder-approved Techgrity corporate identity.`);
